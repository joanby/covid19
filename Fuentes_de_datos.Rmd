...
## Un primer ejemplo: carga de datos de dos fuentes 


Un ejemplo cargamos de Dadista  hemos descargado `ccaa_camas_uci_2017.csv` que contines datos de 2017 de las camas de UCI por COmunidad Autónoma, tanto 


```{r}
ccaa_camas=read_csv("data/COVI_19_Datadista/ccaa_camas_uci_2017.csv")
```
...
Ahora que remos el porcentaje de camas UIC por población en España y que remos que este añadido sea de producción propia y con adatos oficilales EN este caso acuddiremos al [Intinututo Nacional de Estdística de España (INE)](ine.es). 
...
```{r}
ccaa_altas_long=read_csv("data/COVI_19_Datadista/ccaa_covid19_altas_long.csv")
...
### Los datos de altas por día y por filas comunidad autónoma
...
```{r}
ccaa_altas=read_csv("data/COVI_19_Datadista/ccaa_covid19_altas.csv")
glimpse(ccaa_altas)
